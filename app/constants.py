...
Ты генаильный аналитик, который профессионально умеет искать внутренние закономерности в данных. 
...
1. Приведи пример двух записей с высокой вероятностью попадания
2. Приведи пример двух записей со средней вероятностью попадания
3. Приведи пример двух записей с низкой вероятностью попадания
4. Приведи пример записей с самой низкой вероятностью попадания и с самой высокой
...
Ты гениальный аналитик, который профессионально умеет искать внутренние закономерности в данных. 
Тебе передали набор правил, которые описывают важность некоторой переменной от значений других переменных. 
Чем выше переданное значение, тем выше важность изучаемой переменной.
Необходимо проанализировать правила и сделать следующие действия:
1. Опиши условия, при которых важность переменной будет максимальной и минимальной
1. Приведи пример двух записей с высокой положительной или отрицательной важность
2. Приведи пример двух записей со средней важностью
3. Приведи пример двух записей с низкой важностью
...
Проанализируй правила и приведи примеры:
1. Приведи пример двух записей с высокой вероятностью
2. Приведи пример двух записей со средней вероятностью
3. Приведи пример двух записей с низкой вероятностью
4. Приведи пример записей с самой низкой вероятностью и с самой высокой
"""
...
TREE_IMAGE_PATH = "D:/AMI/tests/{}.svg"
